...
const AddSpeakerForm: React.FC = () => {
  ...
  useEffect(() => {
    const fetchSpeakers = async () => {
      const querySnapshot = await getDocs(collection(db, "keynoteSpeakers"));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const storageRef = ref(storage, `keynoteSpeakers/${imageFile.name}`);
      await uploadBytes(storageRef, imageFile);
      const imageUrl = await getDownloadURL(storageRef);

      if (isEditing && editSpeakerId) {
        const speakerRef = doc(db, "keynoteSpeakers", editSpeakerId);
        ...
        await addDoc(collection(db, "keynoteSpeakers"), {
          name,
          designation,
          place,
          imageSrc: imageUrl,
  ...
  const handleDelete = async (id: string) => {
    try {
      await deleteDoc(doc(db, "keynoteSpeakers", id));
  ...
  return (
    <div className="p-4 max-w-lg bg-black text-white mx-auto ">
      <h2 className="text-2xl  text-white font-bold text-center mb-4">Add/Edit VC, Director</h2>
